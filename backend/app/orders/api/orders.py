...
from app.orders.schemas.order import OrderCreate, OrderResponse, OrderUpdate
...
async def get_orders(
    ...
    limit: int = 100,
    user_uuid: str = Query(
        None, description="UUID пользователя для фильтрации"
    ),
    ...
    orders = OrderService(db).get_orders(
        skip=skip, limit=limit, user_uuid=user_uuid
    )
    return orders
