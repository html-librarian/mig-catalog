...
from app.news.schemas.article import ArticleCreate, ArticleUpdate, ArticleResponse
...
async def get_articles(
    ...
    articles  =  ArticleService(db).get_articles(skip = skip, limit = limit, published_only = published_only)
    return articles


@router.get("/{article_id}", response_model = ArticleResponse)
async def get_article(article_id: str  =  Path(..., description = "UUID статьи"), db: Session  =  Depends(get_db)):
...
async def delete_article(article_id: str  =  Path(..., description = "UUID статьи"), db: Session  =  Depends(get_db)):
...
async def search_articles(
    ...
    articles  =  ArticleService(db).search_articles(search_term, skip = skip, limit = limit)
    return articles
