...
class UserService:
    ...
    def create_user(self, user: UserCreate) -> User:
        ...
        if existing_username:
            raise ValueError(
                f"Пользователь с username '{user.username}' уже существует"
            )
        ...
        try:
            ...
            self.db.rollback()
            raise ValueError(
                f"Неожиданная ошибка при создании пользователя: {str(e)}"
            )
    ...
    def update_user(
        ...
        if "email" in update_data:
            ...
            if existing_user and existing_user.uuid != user_id:
                raise ValueError(
                    f"Пользователь с email '{update_data['email']}' "
                    f"уже существует"
                )
        ...
        if "username" in update_data:
            ...
            if existing_username:
                raise ValueError(
                    f"Пользователь с username '{update_data['username']}' "
                    f"уже существует"
                )
